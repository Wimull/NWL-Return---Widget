...
export function FeedbackContentStep({feedbackType, resetFeedback, onFeedBackSent}: FeedbackContentStepType){
    ...
    return(
        ...
        <header className="h-auto">
            <button onClick={resetFeedback} title="Voltar"  className="w-4 h-4 top-5 left-5 absolute text-text-secondary hover:text-text-primary">
                <ArrowLeft weight="bold"/>
            </button>
            <ClosingButton/>
            <span className="text-xl text-text-primary leading-6 px-4 flex gap-1 items-center">
                <img src={selectedFeedbackType.image.source} alt={selectedFeedbackType.image.alt} className="w-6 h-6" />
